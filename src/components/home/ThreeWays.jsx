...
// Pillars can carry their own icon from Strapi now; these are only the
// fallback for entries where an editor hasn't uploaded one, matched by
// position so a renamed title never breaks the icon.
const ICONS = [
  <svg key="0" width="44" height="44" viewBox="0 0 48 48" fill="none" className="text-gold">
  ...
  <svg key="1" width="44" height="44" viewBox="0 0 48 48" fill="none" className="text-gold">
  ...
  <svg key="2" width="44" height="44" viewBox="0 0 48 48" fill="none" className="text-gold">
...
export default function ThreeWays({ page }) {
  const ways = page.threeWays ?? [];
          ...
          {ways.map((w, i) => (
            ...
            >
              <div className="mb-8 transition-transform duration-500 hover:scale-110">
                {w.icon ? (
                  <img src={strapiMediaUrl(w.icon)} alt="" className="h-11 w-11" />
                ) : (
